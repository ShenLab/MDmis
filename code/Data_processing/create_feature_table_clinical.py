...
entire_feature_table.to_csv(os.path.join(data_dir, f"clinical_train_val_intcutoff_{interaction_cutoff}", "feature_table.csv"), index=False)
###

## already done the current one, with 0.4
exit()
entire_feature_table = create_feature_table(
        IDRome_labels, res_data, pair_data,
        aaindex_res_mat,
