...
def train_one_fold(data_dir, data_subdir, models_dir,
                    fold):
...
def main():
    ...
    entire_feature_table = pd.read_csv(
        os.path.join(data_dir, f"clinical_train_val_intcutoff_{interaction_cutoff}", "feature_table.csv"
    )
    )
    ...
    create_folds(feature_table_with_ESM1b, "Cluster", ## split instead by protein cluster
                 os.path.join(data_dir, f"clinical_train_val_intcutoff_{interaction_cutoff}"))
    


    ####
    #run folds in Paralles
    Parallel(n_jobs=6)(delayed(train_one_fold)(data_dir, f"clinical_train_val_intcutoff_{interaction_cutoff}", 
                    os.path.join(models_dir, f"clinical_train_val_intcutoff_{interaction_cutoff}"), 
